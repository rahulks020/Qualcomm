...
public:
  ...
  void assign( K const& keyBegin, K const& keyEnd, V const& val ) {
    ...
    auto beginIndex = m_map.lower_bound(keyBegin);
    if (beginIndex != m_map.begin()) {
      --beginIndex;
    }
    auto result = beginIndex->second == val ?
      beginIndex : m_map.insert_or_assign(beginIndex, keyBegin, val);

    auto endIndex = m_map.lower_bound(keyEnd);

    if (endIndex != m_map.end() && endIndex->second == val)
      endIndex++;

    result++;
    m_map.erase(result, endIndex);

    if (endIndex == m_map.end())
      this->assign(keyEnd, std::numeric_limits<K>::max(), beginIndex->second);
    else
      this->assign(keyEnd, endIndex->first, beginIndex->second);
  }
